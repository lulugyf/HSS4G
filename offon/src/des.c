...
int main1(int argc, char *argv[])
{
	...
	strcpy((char *)key, "0123456789012345");
	flags = 1;

	ret = des_setkey(ctx, key, 16, &flags);

	strcpy(src, "abcd");
	memset(dst, 0, sizeof(dst));
	des_encrypt(ctx, dst, src);

	memset(src, 0, sizeof(src));
	printf("dst[%s]\n", dst);

	des_decrypt(ctx, src, dst);
...
char *hex2bin(char *in, char *out, int *out_len)
{
    char s[3];
    int l;
    int i;
...
void des_11(char *src_str, char *dst_str)
{
	struct des_ctx ctx;
	u8 key[32];
	u32 flags;
    ...
    /*strcpy(s1, "9299F30C8F4D9D5448FCD708CE3C9578");*/
    printf("0 %08X\n", dst_str);
    strcpy(s1, src_str);
    hex2bin(s1, s2, &i);
    printf("hex2bin return [%d]\n", i);

	strcpy(s1, "EF64342A4AF791D3");
    hex2bin(s1, key, &i);
    printf("hex2bin key return [%d]\n", i);
	flags = 1;
	...
	des_decrypt(&ctx, s1, s2);
    des_decrypt(&ctx, s1+8, s2+8);
...
/*int main(int argc, char *argv[])
{
    ...
    strcpy(buf, "9299F30C8F4D9D5448FCD708CE3C9578");
...
int des3_encode(int mode, char *inkey, char *dst, char *src, int len)
{
        struct des3_ede_ctx ctx;
        u8 key[DES3_EDE_KEY_SIZE+1];
        u32 flags[8];
        int ret, i, l, keylen;
        int block_size;
        ...
        keylen = DES3_EDE_KEY_SIZE > strlen(inkey) ? strlen(inkey) : DES3_EDE_KEY_SIZE;
        memset(key, 0, sizeof(key));
        memcpy(key, inkey, keylen);
        flags[0] = 1;
        l = len / block_size + ((len % block_size > 0) ? 1 : 0);
        des3_ede_setkey(&ctx, key, keylen, flags);
        for(i=0; i<l; i++){
                if(mode == 0)
                        des3_ede_encrypt(&ctx, dst+block_size*i, src+block_size*i);
                else
                        des3_ede_decrypt(&ctx, dst+block_size*i, src+block_size*i);
        }
        return block_size*l;
}
